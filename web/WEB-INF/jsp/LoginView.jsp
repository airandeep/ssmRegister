...
<div class="main" >
  ...
  <div class="wrap-login">  <!-- 居中作用 -->
    ...
    <div class="login-part">
      <img src="images/login.PNG" style="width: 400px"/>
      <h3>帐号登录</h3>
      <div class="user-info">


        <form name="fm1" action="judgeLogin" method="post">
          ...
          <c:set var="cFlagLogin" scope="session" value="${0}"/>
          <c:if test="${cFlagLogin == 0}">
            <div id = "test">AIRAN</div>
          </c:if>
          <c:if test="${cFlagLogin == 1}">
            <div id = "test">此为无效账号</div>
          </c:if>
          <c:if test="${cFlagLogin == 2}">
            <div id = "test">密码错误!!!</div>
          </c:if>
          <c:if test="${cFlagLogin == 3}">
            <div id = "test">新用户注册已成功，请登陆</div>
          </c:if>

          <div class="oauth_tips_div">
            <p class="oauth_tips">如果没有账号请注册 <a href="RegisterView" target="_blank">注册</a></p>
          </div>
          <input class="logging" accesskey="l" value="登 录" tabindex="6" type="button" onclick="login()"/>
